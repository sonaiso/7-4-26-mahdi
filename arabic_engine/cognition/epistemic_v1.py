...
def _make_gap(code: DecisionCode, severity: GapSeverity, description: str) -> GapRecord:
    return GapRecord(
        gap_id=f"GAP_{code.name}",
        code=code,
        severity=severity,
...
def validate_episode(inp: KnowledgeEpisodeInput) -> ValidationResult:
    ...
    all ten conditions in order and returns a :class:`ValidationResult`.
    ...
    def _add(code: DecisionCode, severity: GapSeverity, msg: str) -> None:
        codes.append(code)
        gaps.append(_make_gap(code, severity, msg))
        messages.append(msg)
